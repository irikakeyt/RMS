...
<div class="sidebar pe-4 pb-3">
    <nav class="navbar bg-secondary navbar-dark">
        ...
        <div class="navbar-nav w-100">
            ...
            <a href="addroom.php" class="nav-item nav-link active"><i class="fa fa-bed me-2"></i>Rooms</a>
            <div class="nav-item dropdown">
...
<?php
include 'conn.php';
for ($roomNum = 1; $roomNum <= 30; $roomNum++) {
    // Query to check if the room is occupied
    $sql = "SELECT fname, lname, room_price, contact FROM newtenant WHERE room_num = $roomNum";
    ...
    if ($result->num_rows > 0) {
        ...
        $roomNumDB = $roomNum; // Get room number from the database
        echo "<div class='room-panel occupied' style='background-image: url(\"img/o.png\")'>";
        echo "<p>Room $roomNumDB</p>";
        echo "<button onclick='showTenantInfo(\"$fname\", \"$lname\", $roomPrice,\"$contact\", $roomNumDB)'>Show Info</button>";
        echo "</div>";
    } else {
        // Room is vacant
        $roomNumDB = $roomNum; // Get room number from the database
        echo "<div class='room-panel available' style='background-image: url(\"img/av.png\")'>";
        echo "<p>Room $roomNumDB</p>";
        echo "<button onclick='showVacantRoomInfo($roomNumDB)' data-bs-toggle='modal' data-bs-target='#vacantModal'>Show Info</button>";
        echo "</div>";
    }
}
?>
...
<div class="modal fade" id="vacantModal" tabindex="-1" aria-labelledby="vacantModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="background-color: black; border: 2px solid red; color: white; font-family: Georgia;">
            <div class="modal-header">
                <h5 class="modal-title" id="vacantModalLabel">Room Availability</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
<script>
    function showTenantInfo(firstName, lastName, roomPrice, tenantContact, roomNum) {
        // Update modal with tenant information and room price
        document.getElementById("tenantName").innerText = "Tenant: " + firstName + " " + lastName;
        ...
        var tenantModal = new bootstrap.Modal(document.getElementById('tenantModal'));
        tenantModal.show();
    }

    function showVacantRoomInfo(roomNum) {
        // Update modal with room number
        document.getElementById("vacantRoom").innerText = "Room: " + roomNum;
        // Show the vacant room modal
        var vacantModal = new bootstrap.Modal(document.getElementById('vacantModal'));
        vacantModal.show();
    }
</script>
